...
do_drawing (cairo_t * cr, Simulation * self)
{
  ...
  cairo_set_source_surface (cr, car_image,
      (double) (car_get_x (self->car) -
          CAR_SIZE / 2) * ((double) width / (double) CAR_SIZE),
      (600 - CAR_SIZE) * ((double) height / (double) CAR_SIZE));
  cairo_paint (cr);
  cairo_restore (cr);
  ...
  for (; asteroids != NULL; asteroids = asteroids->next) {
    ...
    if (600 - dist > 0) {
      ...
      xs = 1 / ((double) width / ((double) current->size *
              (double) MULTIPLIER));
      ys = 1 / ((double) height / ((double) current->size *
              (double) MULTIPLIER));
      cairo_save (cr);
      cairo_scale (cr, xs, ys);
      cairo_set_source_surface (cr, asteroid_image,
          (double) current->x * ((double) width / ((double) current->size *
                  (double) MULTIPLIER)),
          (dist -
              current->size * MULTIPLIER) * ((double) height /
              ((double) current->size * (double) MULTIPLIER)));
      cairo_paint (cr);
      cairo_restore (cr);
    } else {
      break;
...
static gboolean
on_draw_event (GtkWidget * widget, cairo_t * cr, Simulation * self)
{
  do_drawing (cr, self);
...
static void
key_released (GtkWidget * widget, GdkEvent * event, Car * car)
{
...
static void
key_pressed (GtkWidget * widget, GdkEvent * event, Car * car)
{
  if (((GdkEventKey *) event)->keyval == GDK_KEY_Up) {
...
main (int argc, char *argv[])
{
  ...
  gtk_box_pack_start (GTK_BOX (vbox), simulation->battlefield_display, FALSE,
      FALSE, 0);
