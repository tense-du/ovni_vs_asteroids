...
create_asteroids (Battlefield * self)
{
  ...
  for (tmp = self->priv->asteroids; tmp; tmp = tmp->next) {
    ...
    g_print ("A x: %d, y: %d, asteroide de taille %d\n", current_asteroid->x,
        current_asteroid->y, current_asteroid->size);
  }
}
...
battlefield_check_overlap (Battlefield * self, Asteroid* asteroid)
{
  ...
  if (asteroid->x + asteroid->size * MULTIPLIER < car_x - CAR_SIZE / 2
      || car_x + CAR_SIZE / 2 < asteroid->x || car_y + CAR_SIZE < asteroid->y
      || asteroid->y + asteroid->size * MULTIPLIER < car_y) {
    return;
  } else {
    asteroid->taken = TRUE;
    return;
